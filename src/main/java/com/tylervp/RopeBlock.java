...
public class RopeBlock extends Block implements Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        BlockState blockStateROPEMID = moreblocksmod.ROPEMID.getDefaultState();
        ...
        if(blockStateUp.isOf(this)){
            world4.setBlockState(currentBlockPos.up(), moreblocksmod.ROPEMID.getDefaultState().with(Properties.WATERLOGGED, fluidState6.getFluid() == Fluids.WATER));
        }
    ...
    public boolean canReplace(BlockState state, ItemPlacementContext ctx) {
        if (ctx.getStack().getItem() == this.asItem() && !(ctx.getSide() == Direction.DOWN)){
            ...
            if(!(blockStateDown.isOf(Blocks.AIR) || blockStateDown.isOf(Blocks.WATER)) && blockStateCurrent.isOf(this) && !blockStateDown.isOf(moreblocksmod.ROPEMID) && !blockStateDown.isOf(this)){
                BlockState blockStateROPEMID = moreblocksmod.ROPEMID.getDefaultState();
                World world4 = ctx.getWorld();
                ...
                FluidState fluidState5 = ctx.getWorld().getFluidState(currentBlockPos);
                if((!blockStateNorth.isOf(this) && !blockStateNorth.isOf(moreblocksmod.ROPEMID) && ctx.getSide() == Direction.NORTH) || (!blockStateSouth.isOf(this) && !blockStateSouth.isOf(moreblocksmod.ROPEMID) && ctx.getSide() == Direction.SOUTH) || (!blockStateEast.isOf(this) && !blockStateEast.isOf(moreblocksmod.ROPEMID) && ctx.getSide() == Direction.EAST) || (!blockStateWest.isOf(this) && !blockStateWest.isOf(moreblocksmod.ROPEMID) && ctx.getSide() == Direction.WEST)) {
