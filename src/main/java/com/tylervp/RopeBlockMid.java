...
public class RopeBlockMid extends Block {
    ...
    public boolean canReplace(BlockState state, ItemPlacementContext ctx) {
        if (ctx.getStack().getItem() == moreblocksmod.ROPE.asItem() && !(ctx.getSide() == Direction.DOWN)){
            ...
            BlockState blockStateROPE = moreblocksmod.ROPE.getDefaultState();
            World world4 = ctx.getWorld();
            
            if(!(blockStateDown.isOf(Blocks.AIR) || blockStateDown.isOf(Blocks.WATER)) && !blockStateDown.isOf(Blocks.WATER) && blockStateCurrent.isOf(this) && !blockStateDown.isOf(this) && !blockStateDown.isOf(moreblocksmod.ROPE)){
                ...
                if((!blockStateNorth.isOf(this) && !blockStateNorth.isOf(moreblocksmod.ROPE) && ctx.getSide() == Direction.NORTH) || (!blockStateSouth.isOf(this) && !blockStateSouth.isOf(moreblocksmod.ROPE) && ctx.getSide() == Direction.SOUTH) || (!blockStateEast.isOf(this) && !blockStateEast.isOf(moreblocksmod.ROPE) && ctx.getSide() == Direction.EAST) || (!blockStateWest.isOf(this) && !blockStateWest.isOf(moreblocksmod.ROPE) && ctx.getSide() == Direction.WEST)){
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        if (!state.canPlaceAt(world, pos)) {
            ...
            return moreblocksmod.ROPE.getDefaultState().with(Properties.WATERLOGGED, fluidState5.getFluid() == Fluids.WATER);
    }
