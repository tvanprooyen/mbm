...
public class ThinLogBlock extends PillarBlock implements Waterloggable {
    ...
    protected ThinLogBlock (AbstractBlock.Settings settings) {
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        if(state.get(LEAVES)){
            ...
            if(axis == Axis.X){
                return ThinLogBlock.SHAPEX;

            } else if(axis == Axis.Y){
                return ThinLogBlock.SHAPEY;

            } else if(axis == Axis.Z){
                return ThinLogBlock.SHAPEZ;
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        if(state.get(ThinLogBlock.AXIS) != Axis.Y && (world.getBlockState(pos.down()).isOf(Blocks.CHAIN) || world.getBlockState(pos.down()).isOf(Blocks.LANTERN))){
            ...
            return (BlockState)this.getDefaultState().with(LEAVES, state.get(LEAVES)).with(ThinLogBlock.PRESISTANT, state.get(ThinLogBlock.PRESISTANT)).with(AXIS, state.get(AXIS)).with(Properties.WATERLOGGED, state.get(WATERLOGGED)).with(CHAIN, lHanging).with(ROPE, false);
        } else {

            if(state.get(ThinLogBlock.AXIS) != Axis.Y && (world.getBlockState(pos.down()).isOf(MBMBlocks.ROPE) || world.getBlockState(pos.down()).isOf(MBMBlocks.ROPEMID))){
                return (BlockState)this.getDefaultState().with(ThinLogBlock.LEAVES, state.get(ThinLogBlock.LEAVES)).with(ThinLogBlock.PRESISTANT, state.get(ThinLogBlock.PRESISTANT)).with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(Properties.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)).with(ThinLogBlock.CHAIN, false).with(ThinLogBlock.ROPE, true);
            }
            else{
                return (BlockState)this.getDefaultState().with(ThinLogBlock.LEAVES, state.get(ThinLogBlock.LEAVES)).with(ThinLogBlock.PRESISTANT, state.get(ThinLogBlock.PRESISTANT)).with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(Properties.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)).with(ThinLogBlock.CHAIN, false).with(ThinLogBlock.ROPE, false);
            }
...
/*
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if(state.isOf(this)){
            if (playerItem.getItem() == Items.SHEARS && state.get(ThinLogBlock.LEAVES)) {
                ...
                if(state.isOf(MBMBlocks.THIN_OAK_LOG)){
                    world.setBlockState(pos, MBMBlocks.THIN_STRIPPED_OAK_LOG.getDefaultState().with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.LEAVES, false).with(ThinLogBlock.PRESISTANT, false).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)));
                    Block.dropStack(world, pos, new ItemStack(MBMItems.OAK_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(MBMBlocks.THIN_SPRUCE_LOG)){
                    world.setBlockState(pos, MBMBlocks.THIN_STRIPPED_SPRUCE_LOG.getDefaultState().with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.LEAVES, false).with(ThinLogBlock.PRESISTANT, false).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)));
                    Block.dropStack(world, pos, new ItemStack(MBMItems.SPRUCE_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(MBMBlocks.THIN_ACACIA_LOG)){
                    world.setBlockState(pos, MBMBlocks.THIN_STRIPPED_ACACIA_LOG.getDefaultState().with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.LEAVES, false).with(ThinLogBlock.PRESISTANT, false).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)));
                    Block.dropStack(world, pos, new ItemStack(MBMItems.ACACIA_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(MBMBlocks.THIN_BIRCH_LOG)){
                    world.setBlockState(pos, MBMBlocks.THIN_STRIPPED_BIRCH_LOG.getDefaultState().with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.LEAVES, false).with(ThinLogBlock.PRESISTANT, false).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)));
                    Block.dropStack(world, pos, new ItemStack(MBMItems.BIRCH_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(MBMBlocks.THIN_DARK_OAK_LOG)){
                    world.setBlockState(pos, MBMBlocks.THIN_STRIPPED_DARK_OAK_LOG.getDefaultState().with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.LEAVES, false).with(ThinLogBlock.PRESISTANT, false).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)));
    ...
    public boolean canReplace(BlockState state, ItemPlacementContext context) {
        if(state.get(ThinLogBlock.LEAVES) == false){
            return 
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        Boolean isPresistant = state.<Boolean>get((Property<Boolean>)ThinLogBlock.PRESISTANT);
        //if(!hasLeaves){
            /*if(
                ...
                else if(BSnorth.isOf(this)){hasLeavesNORTH = BSnorth.<Boolean>get((Property<Boolean>)ThinLogBlock.LEAVES);}
                ...
                else if(BSeast.isOf(this)){hasLeavesEAST = BSeast.<Boolean>get((Property<Boolean>)ThinLogBlock.LEAVES);}
                ...
                else if(BSsouth.isOf(this)){hasLeavesSOUTH = BSsouth.<Boolean>get((Property<Boolean>)ThinLogBlock.LEAVES);}
                ...
                else if(BSwest.isOf(this)){hasLeavesWEST = BSwest.<Boolean>get((Property<Boolean>)ThinLogBlock.LEAVES);}
                ...
                if((hasLeavesNORTH || hasLeavesEAST || hasLeavesSOUTH || hasLeavesWEST)){
                    world.setBlockState(pos, this.getDefaultState().with(ThinLogBlock.LEAVES, true).with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.PRESISTANT, isPresistant).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)));
                } else if (!(hasLeavesNORTH && hasLeavesEAST && hasLeavesSOUTH && hasLeavesWEST) && !isPresistant) {
                    world.setBlockState(pos, this.getDefaultState().with(ThinLogBlock.LEAVES, false).with(ThinLogBlock.AXIS, state.get(ThinLogBlock.AXIS)).with(ThinLogBlock.PRESISTANT, isPresistant).with(ThinLogBlock.WATERLOGGED, state.get(ThinLogBlock.WATERLOGGED)).with(ThinLogBlock.CHAIN, state.get(ThinLogBlock.CHAIN)).with(ThinLogBlock.ROPE, state.get(ThinLogBlock.ROPE)));
                } 
            //}
        //}
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(ThinLogBlock.AXIS,LEAVES,PRESISTANT,WATERLOGGED,CHAIN, ROPE);
    }
