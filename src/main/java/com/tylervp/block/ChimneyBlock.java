...
public class ChimneyBlock extends PillarBlock implements Waterloggable {
    ...
    public static final BooleanProperty WATERLOGGED, LARGE;
    
    protected ChimneyBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(Properties.WATERLOGGED, false).with(ChimneyBlock.LARGE, false));
    }
    ...
    public VoxelShape getCollisionShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        if(axis == Axis.X){
            if(state.get(ChimneyBlock.LARGE)){
                return ChimneyBlock.OUTLINE_SHAPE_X_CONNECTED;
            }
            return ChimneyBlock.OUTLINE_SHAPE_X;

        } else if(axis == Axis.Y){
            if(state.get(ChimneyBlock.LARGE)){
                return ChimneyBlock.OUTLINE_SHAPE_Y_CONNECTED;
            }
            return ChimneyBlock.OUTLINE_SHAPE_Y;

        } else if(axis == Axis.Z){
            if(state.get(ChimneyBlock.LARGE)){
                return ChimneyBlock.OUTLINE_SHAPE_Z_CONNECTED;
            }
            return ChimneyBlock.OUTLINE_SHAPE_Z;
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(ChimneyBlock.AXIS, ChimneyBlock.WATERLOGGED, ChimneyBlock.LARGE);
    }
    ...
    static {
        ...
        RAY_TRACE_SHAPE_X_CONNECTED = Block.createCuboidShape(0.0, 1.0, 1.0, 16.0, 15.0, 15.0);
        OUTLINE_SHAPE_X_CONNECTED = VoxelShapes.combineAndSimplify(VoxelShapes.fullCube(), ChimneyBlock.RAY_TRACE_SHAPE_X, BooleanBiFunction.ONLY_FIRST);
        ...
        RAY_TRACE_SHAPE_Y_CONNECTED = Block.createCuboidShape(1.0, 0.0, 1.0, 15.0, 16.0, 15.0);
        OUTLINE_SHAPE_Y_CONNECTED = VoxelShapes.combineAndSimplify(VoxelShapes.fullCube(), ChimneyBlock.RAY_TRACE_SHAPE_Y_CONNECTED, BooleanBiFunction.ONLY_FIRST);
        ...
        RAY_TRACE_SHAPE_Z_CONNECTED = Block.createCuboidShape(1.0, 1.0, 0.0, 15.0, 15.0, 16.0);
        OUTLINE_SHAPE_Z_CONNECTED = VoxelShapes.combineAndSimplify(VoxelShapes.fullCube(), ChimneyBlock.RAY_TRACE_SHAPE_Z, BooleanBiFunction.ONLY_FIRST);
       
        AXIS = Properties.AXIS;
        WATERLOGGED = Properties.WATERLOGGED;
        LARGE = BooleanProperty.of("large");
    }
}
