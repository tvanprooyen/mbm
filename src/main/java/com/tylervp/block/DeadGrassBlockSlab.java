...
public class DeadGrassBlockSlab extends SlabBlock {

   public DeadGrassBlockSlab(AbstractBlock.Settings settings) {
      super(settings);
   ...
   private static boolean canSurvive(BlockState state, WorldView world, BlockPos pos) {
    ...
    BlockState blockState = world.getBlockState(blockPos);
     ...
     if (blockState.isOf(Blocks.SNOW) && (Integer)blockState.get(SnowBlock.LAYERS) == 1) {
        ...
        if(state.isOf(MBMBlocks.DEAD_GRASS_BLOCK_SLAB)) {
...
public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
    ...
    if (!canSurvive(state, world, pos) /* || state.get(WATERLOGGED) */) {
        //world.setBlockState(pos, Blocks.DIRT.getDefaultState());
        if(world.getBlockState(pos).isOf(MBMBlocks.DEAD_GRASS_BLOCK)){
            world.setBlockState(pos, Blocks.DIRT.getDefaultState());
        } 
            ...
            for(int i = 0; i < 4; ++i) {
                ...
                if ((world.getBlockState(blockPos).isOf(MBMBlocks.DIRT_SLAB) || world.getBlockState(blockPos).isOf(Blocks.DIRT)) && canSpread(blockState, world, blockPos)) {
                    ...
                    if(world.getBlockState(blockPos).isOf(Blocks.DIRT)){
                        world.setBlockState(blockPos, MBMBlocks.DEAD_GRASS_BLOCK.getDefaultState());
                        return;
                    }  else {
