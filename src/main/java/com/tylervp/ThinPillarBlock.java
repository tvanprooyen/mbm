...
public class ThinPillarBlock extends PillarBlock implements Waterloggable {
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        if(state.get(ThinPillarBlock.AXIS) != Axis.Y && (world.getBlockState(pos.down()).isOf(Blocks.CHAIN) || world.getBlockState(pos.down()).isOf(Blocks.LANTERN))){
            ...
            return (BlockState)this.getDefaultState().with(LEAVES, state.get(LEAVES)).with(AXIS, state.get(AXIS)).with(Properties.WATERLOGGED, state.get(WATERLOGGED)).with(CHAIN, lHanging).with(ROPE, false);
        } else {

            if(state.get(ThinPillarBlock.AXIS) != Axis.Y && (world.getBlockState(pos.down()).isOf(moreblocksmod.ROPE) || world.getBlockState(pos.down()).isOf(moreblocksmod.ROPEMID))){
                return (BlockState)this.getDefaultState().with(LEAVES, state.get(LEAVES)).with(AXIS, state.get(AXIS)).with(Properties.WATERLOGGED, state.get(WATERLOGGED)).with(CHAIN, false).with(ROPE, true);
            }
            else{
                return (BlockState)this.getDefaultState().with(LEAVES, state.get(LEAVES)).with(AXIS, state.get(AXIS)).with(Properties.WATERLOGGED, state.get(WATERLOGGED)).with(CHAIN, false).with(ROPE, false);
            }
...
/*
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if(state.isOf(this)){
            if (playerItem.getItem() == Items.SHEARS && state.get(LEAVES)) {
                ...
                if(state.isOf(moreblocksmod.THIN_OAK_LOG)){
                    world.setBlockState(pos, moreblocksmod.THIN_STRIPPED_OAK_LOG.getDefaultState().with(AXIS, state.get(AXIS)));
                    Block.dropStack(world, pos, new ItemStack(moreblocksmod.OAK_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(moreblocksmod.THIN_SPRUCE_LOG)){
                    world.setBlockState(pos, moreblocksmod.THIN_STRIPPED_SPRUCE_LOG.getDefaultState().with(AXIS, state.get(AXIS)));
                    Block.dropStack(world, pos, new ItemStack(moreblocksmod.SPRUCE_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(moreblocksmod.THIN_ACACIA_LOG)){
                    world.setBlockState(pos, moreblocksmod.THIN_STRIPPED_ACACIA_LOG.getDefaultState().with(AXIS, state.get(AXIS)));
                    Block.dropStack(world, pos, new ItemStack(moreblocksmod.ACACIA_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(moreblocksmod.THIN_BIRCH_LOG)){
                    world.setBlockState(pos, moreblocksmod.THIN_STRIPPED_BIRCH_LOG.getDefaultState().with(AXIS, state.get(AXIS)));
                    Block.dropStack(world, pos, new ItemStack(moreblocksmod.BIRCH_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(moreblocksmod.THIN_DARK_OAK_LOG)){
                    world.setBlockState(pos, moreblocksmod.THIN_STRIPPED_DARK_OAK_LOG.getDefaultState().with(AXIS, state.get(AXIS)));
                    Block.dropStack(world, pos, new ItemStack(moreblocksmod.DARK_OAK_BARK_FRAGMENT.asItem(), 2));
                } else if(state.isOf(moreblocksmod.THIN_JUNGLE_LOG)){
                    world.setBlockState(pos, moreblocksmod.THIN_STRIPPED_JUNGLE_LOG.getDefaultState().with(AXIS, state.get(AXIS)));
                    Block.dropStack(world, pos, new ItemStack(moreblocksmod.JUNGLE_BARK_FRAGMENT.asItem(), 2));
                }
                
                //Damage Tool as it was used
                if(
                    !state.isOf(moreblocksmod.THIN_STRIPPED_OAK_LOG) || 
                    !state.isOf(moreblocksmod.THIN_STRIPPED_SPRUCE_LOG) || 
                    !state.isOf(moreblocksmod.THIN_STRIPPED_ACACIA_LOG) || 
                    !state.isOf(moreblocksmod.THIN_STRIPPED_BIRCH_LOG) || 
                    !state.isOf(moreblocksmod.THIN_STRIPPED_DARK_OAK_LOG) || 
                    !state.isOf(moreblocksmod.THIN_STRIPPED_JUNGLE_LOG)
                ){
                    playerItem.<PlayerEntity>damage(1, player, (p) -> p.sendToolBreakStatus(hand));
                }
                finished = true;
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return (BlockState)this.getDefaultState().with(LEAVES, false).with(AXIS, ctx.getSide().getAxis()).with(Properties.WATERLOGGED, fuildstate.getFluid() == Fluids.WATER).with(CHAIN, (ctx.getPlayerFacing().getAxis() != Axis.Y && lHanging)).with(ROPE, ctx.getPlayerFacing().getAxis() != Axis.Y && (world.getBlockState(pos.down()).isOf(moreblocksmod.ROPE) || world.getBlockState(pos.down()).isOf(moreblocksmod.ROPEMID)));
    }
    ...
    public boolean canReplace(BlockState state, ItemPlacementContext context) {
        if(state.get(ThinPillarBlock.LEAVES) == false){
            return 
            (context.getStack().getItem() == Items.OAK_LEAVES && state.isOf(moreblocksmod.THIN_OAK_LOG)) || 
            (context.getStack().getItem() == Items.SPRUCE_LEAVES && state.isOf(moreblocksmod.THIN_SPRUCE_LOG)) || 
            (context.getStack().getItem() == Items.ACACIA_LEAVES && state.isOf(moreblocksmod.THIN_ACACIA_LOG)) || 
            (context.getStack().getItem() == Items.BIRCH_LEAVES && state.isOf(moreblocksmod.THIN_BIRCH_LOG)) || 
            (context.getStack().getItem() == Items.DARK_OAK_LEAVES && state.isOf(moreblocksmod.THIN_DARK_OAK_LOG)) || 
            (context.getStack().getItem() == Items.JUNGLE_LEAVES && state.isOf(moreblocksmod.THIN_JUNGLE_LOG));
        }
        return false;
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        //if(!hasLeaves){
            /*if(
                ...
                if(
                    (BSnorth.isOf(Blocks.OAK_LEAVES) && state.isOf(moreblocksmod.THIN_OAK_LOG)) || 
                    (BSnorth.isOf(Blocks.SPRUCE_LEAVES) && state.isOf(moreblocksmod.THIN_SPRUCE_LOG)) || 
                    (BSnorth.isOf(Blocks.ACACIA_LEAVES) && state.isOf(moreblocksmod.THIN_ACACIA_LOG)) || 
                    (BSnorth.isOf(Blocks.BIRCH_LEAVES) && state.isOf(moreblocksmod.THIN_BIRCH_LOG)) || 
                    (BSnorth.isOf(Blocks.DARK_OAK_LEAVES) && state.isOf(moreblocksmod.THIN_DARK_OAK_LOG)) || 
                    (BSnorth.isOf(Blocks.JUNGLE_LEAVES) && state.isOf(moreblocksmod.THIN_JUNGLE_LOG))
                ) { hasLeavesNORTH = true; } 
                else if(BSnorth.isOf(this)){hasLeavesNORTH = BSnorth.<Boolean>get((Property<Boolean>)ThinPillarBlock.LEAVES);}

                
                if(
                    (BSeast.isOf(Blocks.OAK_LEAVES) && state.isOf(moreblocksmod.THIN_OAK_LOG)) || 
                    (BSeast.isOf(Blocks.SPRUCE_LEAVES) && state.isOf(moreblocksmod.THIN_SPRUCE_LOG)) || 
                    (BSeast.isOf(Blocks.ACACIA_LEAVES) && state.isOf(moreblocksmod.THIN_ACACIA_LOG)) || 
                    (BSeast.isOf(Blocks.BIRCH_LEAVES) && state.isOf(moreblocksmod.THIN_BIRCH_LOG)) || 
                    (BSeast.isOf(Blocks.DARK_OAK_LEAVES) && state.isOf(moreblocksmod.THIN_DARK_OAK_LOG)) || 
                    (BSeast.isOf(Blocks.JUNGLE_LEAVES) && state.isOf(moreblocksmod.THIN_JUNGLE_LOG))
                ) { hasLeavesEAST = true; }
                else if(BSeast.isOf(this)){hasLeavesEAST = BSeast.<Boolean>get((Property<Boolean>)ThinPillarBlock.LEAVES);}

                
                if(
                    (BSsouth.isOf(Blocks.OAK_LEAVES) && state.isOf(moreblocksmod.THIN_OAK_LOG)) || 
                    (BSsouth.isOf(Blocks.SPRUCE_LEAVES) && state.isOf(moreblocksmod.THIN_SPRUCE_LOG)) || 
                    (BSsouth.isOf(Blocks.ACACIA_LEAVES) && state.isOf(moreblocksmod.THIN_ACACIA_LOG)) || 
                    (BSsouth.isOf(Blocks.BIRCH_LEAVES) && state.isOf(moreblocksmod.THIN_BIRCH_LOG)) || 
                    (BSsouth.isOf(Blocks.DARK_OAK_LEAVES) && state.isOf(moreblocksmod.THIN_DARK_OAK_LOG)) || 
                    (BSsouth.isOf(Blocks.JUNGLE_LEAVES) && state.isOf(moreblocksmod.THIN_JUNGLE_LOG))
                ) { hasLeavesSOUTH = true; }
                else if(BSsouth.isOf(this)){hasLeavesSOUTH = BSsouth.<Boolean>get((Property<Boolean>)ThinPillarBlock.LEAVES);}

                
                if(
                    (BSwest.isOf(Blocks.OAK_LEAVES) && state.isOf(moreblocksmod.THIN_OAK_LOG)) || 
                    (BSwest.isOf(Blocks.SPRUCE_LEAVES) && state.isOf(moreblocksmod.THIN_SPRUCE_LOG)) || 
                    (BSwest.isOf(Blocks.ACACIA_LEAVES) && state.isOf(moreblocksmod.THIN_ACACIA_LOG)) || 
                    (BSwest.isOf(Blocks.BIRCH_LEAVES) && state.isOf(moreblocksmod.THIN_BIRCH_LOG)) || 
                    (BSwest.isOf(Blocks.DARK_OAK_LEAVES) && state.isOf(moreblocksmod.THIN_DARK_OAK_LOG)) || 
                    (BSwest.isOf(Blocks.JUNGLE_LEAVES) && state.isOf(moreblocksmod.THIN_JUNGLE_LOG))
                ) { hasLeavesWEST = true; }
                ...
                if((hasLeavesNORTH || hasLeavesEAST || hasLeavesSOUTH || hasLeavesWEST)){
                    world.setBlockState(pos, this.getDefaultState().with(LEAVES, true).with(AXIS, state.get(AXIS)).with(PRESISTANT, state.get(PRESISTANT)).with(WATERLOGGED, state.get(WATERLOGGED)).with(CHAIN, state.get(CHAIN)).with(ROPE, state.get(ROPE)));
                } else if (!(hasLeavesNORTH && hasLeavesEAST && hasLeavesSOUTH && hasLeavesWEST) && !isPresistant) {
                    world.setBlockState(pos, this.getDefaultState().with(LEAVES, false).with(AXIS, state.get(AXIS)).with(PRESISTANT, state.get(PRESISTANT)).with(WATERLOGGED, state.get(WATERLOGGED)).with(CHAIN, state.get(CHAIN)).with(ROPE, state.get(ROPE)));
                } 
            //}
        //}
